# SVC_UFDubProvider.md

## Purpose
UFDub - ??????? ???????????? ????????? ?? ???????? ???????? Cloudstream provider module delivering Ukrainian-localized streams for this source.

## Current Implementation Status
? **IMPLEMENTED** - Kotlin `MainAPI` provider module present.

## Implementation Details
- **Location**: UFDubProvider\src\main\kotlin\com\lagradost\UFDubProvider.kt; UFDubProvider\src\main\kotlin\com\lagradost\UFDubProviderPlugin.kt; UFDubProvider/build.gradle.kts
