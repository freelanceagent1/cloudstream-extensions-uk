# SVC_UATuTFunProvider.md

## Purpose
UATuTFun - ?????????? ?? ???????????? ??????-????????? Cloudstream provider module delivering Ukrainian-localized streams for this source.

## Current Implementation Status
? **IMPLEMENTED** - Kotlin `MainAPI` provider module present.

## Implementation Details
- **Location**: UATuTFunProvider\src\main\kotlin\com\lagradost\UATuTFunProvider.kt; UATuTFunProvider\src\main\kotlin\com\lagradost\UATuTProviderPlugin.kt; UATuTFunProvider/build.gradle.kts
